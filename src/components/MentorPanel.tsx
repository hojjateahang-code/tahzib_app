...
export function MentorPanel() {
  ...
  const pendingTasksCount = useLiveQuery(
    async () => {
      ...
      return tasks.filter(t => {
        const isTarget = t.roleTarget === 'MENTOR' || t.roleTarget === 'ALL' || t.assignedTo === currentUser.id;
  ...
  const getBadge = (tabId: string) => {
    ...
    if (tabId === 'DASHBOARD' && pendingTasksCount && pendingTasksCount > 0) return pendingTasksCount;
    return 0;
  };
